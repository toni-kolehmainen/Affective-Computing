...
parser.add_argument(
    "--save_dir", type=str, default="youtube", help="the dir to save the dataset"
)
parser.add_argument(
    "--video_meta",
    type=str,
    default="youtube_playlist_tv.json",
...
parser.add_argument("--fps", type=int, default=24, help="output fps for videos")
parser.add_argument("--n_workers", type=int, default=1, help="number of workers")

args = parser.parse_args()


def get_youtube_video_with_transcript(
    ytid, save_dir="./youtube", transcript_only=False
):
    # where to save
...
def filter_frames_by_subs(subs_file, fps=24):
    ...
    useful_frames = np.concatenate(
        [
            list(
                range(
                    int(sub_range["start"] * fps),
...
def video_to_frames(ytid, save_dir="./youtube", fps=24):
    print("Inside video to frames")
    if "https://www.youtube.com/watch" in ytid:
        ytid = ytid.split("?v=")[-1]
    save_path = save_dir #os.path.join(save_dir, ytid)
    print("Save path", save_path)
    video_file = os.path.join(
        save_path, list(filter(lambda x: ".mp4" in x, os.listdir(save_path)))[0]
    )
    video_name = video_file.replace(".mp4", "")
    #frames_dir = os.path.join(save_path, video_name, "frames")
    frames_dir = os.path.join(save_path, "frames")
    if not os.path.exists(frames_dir):
        print("Creating folder", frames_dir)
        os.mkdir(frames_dir)
    command = [
        "ffmpeg",
        "-i",
...
def meta_to_frames(
    meta, save_dir="./youtube", fps=1, transcript_only=False, download=True
):
    # if download:
    ...
    if success:
        print("Meta", meta)
        save_path, frames_dir, video_file, status = video_to_frames(
            meta["link"], save_dir=save_dir, fps=fps
        )
        if status != 0:
